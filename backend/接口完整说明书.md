...
### 5.1 调整断句参数（批量设置）
**接口**: `POST /api/breaking-sentences/settings?taskid={task_id}`
...
### 5.2 调整单个断句参数
**接口**: `POST /api/breaking-sentences/settings?breaking_sentence_id={breaking_sentence_id}`
...
**说明**:
- `rule_type`: 规范类型，1-数字英文，2-音标调整，3-专有词汇
- `scope`: 1-全局，2-任务
- `task_id`: scope为2时必填
...
### 6.2 获取阅读规范列表
**接口**: `GET /api/reading-rules?task_id=1&scope=2`
...
### 6.3 应用阅读规范 ⭐（需补充）
**接口**: `POST /api/reading-rules/apply?ruleid={rule_id}&taskid={task_id}`
...
### 7.1 合并音频
**接口**: `POST /api/tasks/audio/merge?taskid={task_id}`
...
### 7.2 查询合并状态
**接口**: `GET /api/audio/merges/merge?mergeid={merge_id}`
...
### 8.1 获取音色列表
**接口**: `GET /api/voices?is_recommended=1&language=zh-CN`
...
### 9.1 获取断句标准
**接口**: `GET /api/sentence-breaking/standards`
...
### 9.2 保存断句标准设置 ⭐（需补充）
**接口**: `POST /api/sentence-breaking/settings?taskid={task_id}`
