...
### 2.3 获取任务列表 ⭐（需补充）
**接口**: `GET /tasks?page=1&page_size=20&status=2`
...
### 3.1 获取断句列表
**接口**: `GET /tasks/breaking-sentences?taskid={task_id}`
...
### 3.2 获取断句详情
**接口**: `GET /breaking-sentences/info?breaking_sentence_id={breaking_sentence_id}`
...
### 3.3 删除断句
**接口**: `DELETE /breaking-sentences?breaking_sentence_id={breaking_sentence_id}`
...
### 4.1 合成单个断句
**接口**: `POST /breaking-sentences/synthesize?breaking_sentence_id={breaking_sentence_id}`
...
### 4.2 重新合成断句
**接口**: `POST /breaking-sentences/resynthesize?breaking_sentence_id={breaking_sentence_id}`
...
### 4.3 批量合成
**接口**: `POST /tasks/synthesize?taskid={task_id}`
...
### 4.4 查询合成状态
**接口**: `GET /synthesis/tasks?taskid={task_id}`
